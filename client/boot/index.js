...
function reduxStoreReady( reduxStore ) {
	...
	page( '*', function( context, next ) {
		...
		// If `?tour` is present, show the default guides tour
		if ( config.isEnabled( 'guidestours' ) && context.query.tour ) {
			context.store.dispatch( showGuidesTour( { show: true } ) );
			//setTimeout( context.store.dispatch.bind( this, showGuidesTour( { show: false } ) ), 5000 );
		}
